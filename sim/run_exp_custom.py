...
import numpy as np
sys.path.append('./')
from your_code import objective, video
from absl import app
from sim import network
from sim.env import CustomEnv
from sim.generate_plts import generate_plts
import utils
from your_code.custom import CustomAlg
...
def main(argv):
  ...
  # Mapping bitrate (kbps) to perceptual quality
  ...
  # Note: Created a CustomEnv object instead of previous Env object
  env = CustomEnv(vid, obj, net)
  obj_client = copy.deepcopy(obj)
  vid_client = copy.deepcopy(vid)

  # Init Custom Algorithm
  abr_alg = CustomAlg(vid_client, obj_client, remaining_args)
  ...
  # Similation still runs in steps, however, the algorithm can choose to transmit more than one chunks
  for i in range(vid.num_max_chunks()):
    ...
    chunks, qualities = abr_alg.next_quality(**feedback)

    ttd, rebuf_sec, smooth_pen, prev_chunk_rate = env.step(chunks, qualities)
