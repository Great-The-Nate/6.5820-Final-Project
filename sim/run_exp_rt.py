...
def main(argv):
    ...
    # Updated to use different algorithms by adding flags
    if args.bba:
    ...
    while env.client_is_streaming():
        ...
        # Obtain the chunk index and quality to either send fresh or retransmit
        throughput = abr_alg.estimate_throughput()
        chunk_index = env.chunk_to_transmit(throughput)
        
        feedback["chunk_index"] = chunk_index
         
        quality = abr_alg.next_quality(**feedback)
        ttd, rebuf_sec, smooth_pen, prev_chunk_rate = env.step(chunk_index, quality)
